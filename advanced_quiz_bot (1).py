...
# -*- coding: utf-8 -*-
"""
Advanced Quiz Bot — fully fixed overlay on top of bot_base.py
Fixes:
 • HTML Export "name 'title' is not defined" error
 • Math / LaTeX rendering via MathJax CDN in all HTML exports
 • Bengali text rendering in HTML exports
 • Full draft-panel with: Open Draft · Set Active · Prefix · Shuffle ·
   Edit Title/Time/Negative · Manage Questions · Sections · HTML Export ·
   Practice Link · Delete · Draft Browser
 • Faster, error-free exam flow with pause / resume / skip / speed
 • Improved personal result DM (accuracy, percentile, section breakdown)
...
base._FINAL_SUPPORTED_GROUP_COMMANDS = set(
    getattr(base, "_FINAL_SUPPORTED_GROUP_COMMANDS", set())
) | {"pauseq", "resumeq", "skipq", "speed"}
...
def existing_question_signatures(draft_id: str) -> set:
    seen: set = set()
...
def dedup_add_question_to_draft(
    draft_id: str, question: str, options: List[str], correct_option: int, explanation: str, src: str
) -> Tuple[bool, Optional[int]]:
    sig = question_signature(question, options)
    if sig in existing_question_signatures(draft_id):
        return False, None
    q_no = base.add_question_to_draft(
        draft_id,
        clean_forwarded_text(question),
        [clean_forwarded_text(o) for o in options],
        int(correct_option),
        clean_forwarded_text(explanation),
        src,
    )
    return True, q_no
...
def list_sections(draft_id: str) -> List[Any]:
    return base.DBH.fetchall(
        "SELECT * FROM draft_sections WHERE draft_id=? ORDER BY section_no ASC", (draft_id,)
    )


def set_section(draft_id: str, start_q: int, end_q: int, title: str, question_time: Optional[int]) -> None:
    next_no_row = base.DBH.fetchone(
        "SELECT COALESCE(MAX(section_no), 0) AS mx FROM draft_sections WHERE draft_id=?", (draft_id,)
    )
    ...
    base.DBH.execute(
        ...
        (draft_id, next_no, base.normalize_visual_text(title), int(start_q), int(end_q),
         int(question_time) if question_time else None),
    )
...
def apply_sections_to_session(session_id: str, draft_id: str) -> None:
    for row in list_sections(draft_id):
        base.DBH.execute(
            "UPDATE session_questions SET section_title=?, question_time_override=? "
            "WHERE session_id=? AND q_no BETWEEN ? AND ?",
            (row["title"], row["question_time"], session_id, int(row["start_q"]), int(row["end_q"])),
        )


def start_clone_session(user_id: int, draft_id: str, clone_token: str, source_text: str) -> None:
    base.DBH.execute(
        "INSERT OR REPLACE INTO clone_sessions"
        "(user_id, draft_id, clone_token, source_text, active, created_at, updated_at)"
        " VALUES(?,?,?,?,1,COALESCE((SELECT created_at FROM clone_sessions WHERE user_id=?),?),?)",
...
def format_draft_info(draft: Any) -> str:
    ...
    title_prefix = "ON" if int(draft.get("show_title_prefix") or 1) else "OFF"
    lines = [
        "<b>Draft Info</b>",
...
def delete_question_numbers(draft_id: str, q_numbers: List[int]) -> int:
    ...
    with closing(base.DBH.connect()) as conn:
        ...
        for q_no in sorted(set(int(x) for x in q_numbers), reverse=True):
            cur = conn.execute(
                "DELETE FROM draft_questions WHERE draft_id=? AND q_no=?", (draft_id, q_no)
            )
            removed += int(cur.rowcount or 0)
        rows = conn.execute(
            "SELECT id, q_no FROM draft_questions WHERE draft_id=? ORDER BY q_no ASC", (draft_id,)
        ).fetchall()
...
def shuffle_draft_questions(draft_id: str) -> None:
    ...
    with closing(base.DBH.connect()) as conn:
        ...
        for idx, row in enumerate(rows, start=1):
            conn.execute(
                "INSERT INTO draft_questions(draft_id, q_no, question, options, correct_option, explanation, src)"
                " VALUES(?,?,?,?,?,?,?)",
                (draft_id, idx, row["question"], row["options"], row["correct_option"], row["explanation"], row["src"]),
            )
        conn.commit()
    base.refresh_draft_status(draft_id)
...
# ── HTML export: draft quiz (with MathJax + Bengali) ─────────
def _draft_quiz_html(draft: Any, questions: List[Any]) -> str:
    """Generate a professional HTML quiz webview with MathJax + Bengali support."""
    draft_title = base.normalize_visual_text(str(draft.get("title") or "Quiz"))
    brand = base.html_escape(base.CONFIG.brand_name)
    generated = base.html_escape(base.fmt_dt(base.now_ts()))
    neg = draft.get("negative_mark", 0)
    q_time = draft.get("question_time", 30)
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        correct = int(q.get("correct_option", 0))
        expl = base.html_escape(base.normalize_visual_text(str(q.get("explanation") or "")))
        q_text = base.html_escape(base.normalize_visual_text(str(q.get("question") or "")))

        opt_html = ""
        for oi, opt in enumerate(opts):
            cls = "opt correct" if oi == correct else "opt"
            marker = "✅ " if oi == correct else f"{letter(oi)}. "
            opt_html += f"<div class='{cls}'><span class='ol'>{marker}</span>{base.html_escape(base.normalize_visual_text(str(opt)))}</div>\n"

        expl_block = f"<div class='expl'>💡 {expl}</div>" if expl else ""

        q_blocks.append(f"""
<div class='qcard' id='q{idx}'>
  <div class='qnum'>Q{idx}</div>
...
<title>{base.html_escape(draft_title)}</title>
...
<!-- Google Fonts: Noto Sans Bengali + Noto Sans -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;500;700&family=Noto+Sans:ital,wght@0,400;0,600;0,700;1,400&display=swap">
...
.brand {{ font-size: 12px; font-weight: 700; letter-spacing: .1em;
          text-transform: uppercase; color: var(--accent); }}
.htitle {{ font-size: 22px; font-weight: 700; margin-top: 4px;
           line-height: 1.2; word-break: break-word; }}
.hmeta  {{ display: flex; gap: 14px; flex-wrap: wrap; margin-top: 8px;
           font-size: 12px; color: var(--muted); }}
.hbadge {{ background: rgba(88,166,255,.13); border: 1px solid rgba(88,166,255,.3);
           color: var(--accent); border-radius: 6px; padding: 2px 8px; }}

/* ── progress bar ── */
.progress-wrap {{ height: 3px; background: var(--border); margin-top: 10px; border-radius: 2px; }}
.progress-bar  {{ height: 3px; background: linear-gradient(90deg,#58a6ff,#3fb950);
                  border-radius: 2px; width: 0; transition: width .4s ease; }}

/* ── main layout ── */
.main {{ max-width: 860px; margin: 0 auto; padding: 24px 16px 0; }}
...
.qtext {{
  ...
  color: var(--text); margin-bottom: 16px; word-break: break-word;
}}
...
.ol {{ min-width: 30px; font-weight: 700; color: var(--muted); flex-shrink: 0; }}
...
<div class="header">
  <div class="brand">{brand} • Quiz Export</div>
  <div class="htitle">{base.html_escape(draft_title)}</div>
  <div class="hmeta">
    <span class="hbadge">{total} Questions</span>
    <span class="hbadge">{q_time}s / question</span>
    <span class="hbadge">Negative: -{neg}</span>
    <span>{generated}</span>
  </div>
...
<div class="footer">
  {brand} &nbsp;•&nbsp; {total} questions &nbsp;•&nbsp; Generated {generated}
</div>
</div>
...
(function() {{
  ...
  function upd() {{
    ...
    bar.style.width = (h > 0 ? Math.min(100, s / h * 100) : 100) + '%';
  }}
...
# ── enhanced report HTML (with MathJax + Bengali) ────────────
def _report_html(title_raw: str, summary: Dict[str, Any], ranking: List[Dict[str, Any]], theme: Dict[str, str]) -> str:
    title = base.normalize_visual_text(title_raw) or "Exam"
    brand = base.html_escape(base.CONFIG.brand_name)
    ...
    if not rows_html:
        rows_html = [
            "<tr><td class='center'>—</td><td><div class='primary'>No eligible participants</div></td>"
            "<td class='num ok'>0</td><td class='num bad'>0</td><td class='num skip'>0</td>"
            "<td class='num time'>0s</td><td class='num score'>0.00</td></tr>"
        ]
    ...
    cards_html = "".join(
        f"<div class='card'><div class='k'>{base.html_escape(str(k))}</div>"
        f"<div class='v'>{base.html_escape(str(v))}</div></div>"
        for k, v in cards
    )
...
<title>{base.html_escape(title)} — Report</title>
<script>
window.MathJax = {{
...
<script src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-chtml.js" async></script>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;700&family=Noto+Sans:wght@400;600;700&display=swap">
<style>
*{{box-sizing:border-box;margin:0;padding:0}}
body{{font-family:'Noto Sans Bengali','Noto Sans',system-ui,sans-serif;
     background:linear-gradient(135deg,#07111d,#0d1e33);color:#eef6ff;
     min-height:100vh;padding:0 0 60px;line-height:1.6;}}
.wrap{{max-width:1100px;margin:0 auto;padding:28px 16px}}
.hero{{position:sticky;top:0;background:rgba(7,17,29,.85);backdrop-filter:blur(16px);
      border:1px solid rgba(255,255,255,.07);border-radius:20px;
      padding:20px 24px;box-shadow:0 16px 40px rgba(0,0,0,.3);z-index:20;}}
.brand{{font-size:12px;letter-spacing:.1em;text-transform:uppercase;color:#9ec5f8;font-weight:700;}}
.title{{font-size:30px;font-weight:800;margin-top:8px;line-height:1.15;word-break:break-word;}}
.gen{{font-size:12px;color:#bfd3ee;margin-top:5px;}}
.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:14px;margin-top:20px;}}
.card{{background:rgba(18,37,63,.9);border:1px solid rgba(255,255,255,.08);border-radius:16px;
      padding:14px 16px;box-shadow:0 8px 24px rgba(0,0,0,.22);
      animation:cardIn .45s ease both;transform:translateY(12px);opacity:0;}}
...
.k{{font-size:11px;color:#9db6d3;text-transform:uppercase;letter-spacing:.06em;}}
.v{{font-size:20px;font-weight:800;margin-top:6px;word-break:break-word;}}
.sec-head{{font-size:18px;font-weight:800;margin:24px 0 12px;color:#eef6ff;}}
.tbl-wrap{{overflow:auto;border-radius:18px;border:1px solid rgba(255,255,255,.08);
          background:rgba(9,19,34,.85);box-shadow:0 16px 40px rgba(0,0,0,.25);}}
.tbl{{width:100%;border-collapse:separate;border-spacing:0;min-width:700px;}}
.tbl thead th{{position:sticky;top:0;background:#10243c;color:#fff;padding:13px 12px;
              font-size:12px;text-align:left;z-index:3;}}
.tbl tbody td{{padding:12px 12px;border-top:1px solid rgba(255,255,255,.06);font-size:14px;vertical-align:top;}}
.tbl tbody tr:hover td{{background:rgba(255,255,255,.03);}}
.center{{text-align:center;}}.num{{text-align:right;}}
.ok{{color:#7cf0a1;font-weight:700;}}.bad{{color:#ff8f8f;font-weight:700;}}
.skip{{color:#ffd17b;font-weight:700;}}.time{{color:#bfe0ff;}}.score{{font-weight:800;}}
.primary{{font-size:14px;line-height:1.2;word-break:break-word;}}
.footer{{text-align:center;margin-top:32px;font-size:12px;color:#7090b0;}}
</style>
</head>
<body>
<div class="wrap">
<div class="hero">
  <div class="brand">{brand} • Interactive Report</div>
  <div class="title">{base.html_escape(title)}</div>
  <div class="gen">Generated at {base.html_escape(base.fmt_dt(base.now_ts()))}</div>
</div>
<div class="grid">{cards_html}</div>
<div class="sec-head">Ranking Analysis</div>
<div class="tbl-wrap">
<table class="tbl">
<thead><tr>
  <th style="width:50px">#</th>
  <th>Name</th>
  <th style="width:88px">Correct</th>
  <th style="width:88px">Wrong</th>
  <th style="width:88px">Skipped</th>
  <th style="width:92px">Time</th>
  <th style="width:106px">Score</th>
</tr></thead>
<tbody>{"".join(rows_html)}</tbody>
</table>
</div>
<div class="footer">{brand} &nbsp;•&nbsp; {base.html_escape(title)} &nbsp;•&nbsp; {base.html_escape(base.fmt_dt(base.now_ts()))}</div>
</div>
</body>
</html>"""
...
# ── draft panel builder ───────────────────────────────────────
def _build_draft_browser_list_text_markup(
    user_id: int, page: int = 0, header: str = ""
) -> Tuple[str, InlineKeyboardMarkup]:
    drafts = _visible_drafts_for_user(user_id)
    if not drafts:
        text = (
            (f"{header}\n\n" if header else "") +
            "<b>Your Drafts</b>\n\nYou do not have any drafts yet. Create one with New Exam."
        )
    ...
    for idx, row in enumerate(rows, start=start + 1):
        ...
        lines.append(f"Questions: <b>{row.get('q_count',0)}</b>    Time: <b>{row['question_time']} sec</b>")
        lines.append(f"Negative: <b>{row['negative_mark']}</b>")
    ...
    for row in rows:
        kb_rows.append([
            ...
            InlineKeyboardButton(
                "✅ Active" if row["id"] == active_id else "🔄 Set Active",
                callback_data=f"ap:set:{row['id']}:{page}",
            ),
        ])
    nav: List[InlineKeyboardButton] = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"ap:page:{page-1}"))
...
def _build_draft_detail_text_markup(
    user_id: int, draft_id: str, page: int = 0, header: str = ""
) -> Tuple[str, InlineKeyboardMarkup]:
    draft = base.get_draft(draft_id)
    if not draft:
        return _build_draft_browser_list_text_markup(user_id, page=page, header="⚠️ Draft not found.")
    if (int(draft["owner_id"]) != user_id
            and not getattr(base, "is_all_access_admin", lambda _x: False)(user_id)
            and not base.is_owner(user_id)):
    ...
    practice_url = (
        base._build_practice_url(bot_username, draft_id, int(draft["owner_id"]))
        if bot_username and len(q_rows) > 0 and hasattr(base, "_build_practice_url")
        else None
    )
    ...
    kb_rows: List[List[InlineKeyboardButton]] = [
        [
            InlineKeyboardButton("📂 Open Draft", callback_data=f"ap:view:{draft_id}:{page}"),
            InlineKeyboardButton("🔄 Set Active", callback_data=f"ap:set:{draft_id}:{page}"),
        ],
        [
            InlineKeyboardButton(
                f"⚙️ Prefix {'ON' if int(draft.get('show_title_prefix') or 1) else 'OFF'}",
                callback_data=f"ap:prefix:{draft_id}:{page}",
            ),
        ...
        [
            ...
            InlineKeyboardButton("🧩 Manage Questions", callback_data=f"ap:manageq:{draft_id}:{page}"),
        ],
        [
            InlineKeyboardButton("📚 Sections", callback_data=f"ap:sections:{draft_id}:{page}"),
            InlineKeyboardButton("🌐 HTML Export", callback_data=f"ap:htmlexport:{draft_id}:{page}"),
        ],
        [
            InlineKeyboardButton("🗑 Delete Draft", callback_data=f"ap:del:{draft_id}:{page}"),
            InlineKeyboardButton("📖 Draft Browser", callback_data=f"ap:page:{page}"),
        ],
    ]
    if practice_url:
        kb_rows.insert(5, [InlineKeyboardButton("🧪 Practice Link", url=practice_url)])
...
async def send_draft_card(context, chat_id: int, user_id: int, draft_id: str, header: str = "") -> None:
    ...
    practice_url = (
        base._build_practice_url(bot_username, draft_id, int(draft["owner_id"]))
        if count > 0 and hasattr(base, "_build_practice_url")
        else None
    )
    ...
    if practice_url and practice:
        max_att = int(practice["max_attempts"])
        lines.extend([
            "",
            "<b>Practice Link</b>",
            f"<a href=\"{practice_url}\">Open practice in bot inbox</a>",
            f"Attempts per user: <b>{'Unlimited' if max_att <= 0 else max_att}</b>",
        ])
    text = "\n".join(lines)
    ...
    kb_rows: List[List[InlineKeyboardButton]] = [
        ...
        [
            InlineKeyboardButton("⚙️ Toggle Prefix", callback_data=f"ap:prefix:{draft_id}:0"),
...
# ── callback router ───────────────────────────────────────────
_previous_callback_router = base.callback_router


async def callback_router(update: Update, context) -> None:
    query = update.callback_query
    if not query or not query.data:
        return await _previous_callback_router(update, context)
    ...
    if data == "panel:drafts" or data.startswith("ap:"):
        ...
        if action == "set" and len(parts) >= 4:
            ...
            if int(draft["owner_id"]) != user.id and not getattr(base, "is_all_access_admin", lambda _x: False)(user.id) and not base.is_owner(user.id):
                text, kb = _build_draft_browser_list_text_markup(user.id, page=page, header="⚠️ Access denied.")
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
                return
        ...
        if action in {"title", "time", "neg"} and len(parts) >= 4:
            ...
            prompts = {
                "title": "✏️ Send the new draft title:",
                "time": "⏱ Send the new default time in seconds (e.g. 30):",
                "neg": "➖ Send the new negative mark (e.g. 0.25):",
            }
        ...
        if action == "prefix" and len(parts) >= 4:
            ...
            if not draft:
                ...
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
                return
            new_val = 0 if int(draft.get("show_title_prefix") or 1) else 1
            ...
            text, kb = _build_draft_detail_text_markup(user.id, draft_id, page=page, header=f"✅ Title prefix turned <b>{label}</b>.")
            await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
            return
        ...
        # ── Manage Questions ──────────────────────────────────
        if action == "manageq" and len(parts) >= 4:
            draft_id = parts[2]
            page = int(parts[3]) if parts[3].isdigit() else 0
            draft = base.get_draft(draft_id)
            if not draft:
                ...
                return
            q_rows = base.get_draft_questions(draft_id)
            lines_q: List[str] = [f"<b>Manage Questions</b>", f"Draft: <b>{base.html_escape(base.normalize_visual_text(draft['title']))}</b>", f"Code: <code>{draft_id}</code>", f"Total: <b>{len(q_rows)}</b> questions", ""]
            for q in q_rows[:30]:
                opts = base.jload(q["options"], []) or []
                correct = int(q.get("correct_option", 0))
                opt_letters = "ABCDEFGHIJ"
                corr_letter = opt_letters[correct] if correct < len(opt_letters) else str(correct + 1)
                q_text = base.html_escape(base.normalize_visual_text(str(q["question"]))[:80])
                lines_q.append(f"<b>Q{q['q_no']}.</b> {q_text}{'…' if len(str(q['question'])) > 80 else ''}")
                lines_q.append(f"  Answer: <b>{corr_letter}</b>  | Options: {len(opts)}")
                lines_q.append("")
            if len(q_rows) > 30:
                lines_q.append(f"<i>… and {len(q_rows) - 30} more. Use /delq to remove specific questions.</i>")
            lines_q.append("\nUse <code>/delq {draft_id} 3,5-7</code> to delete question numbers.")
            kb_q = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back to Draft", callback_data=f"ap:view:{draft_id}:{page}")]])
            await base.panel_show_message(query.message, user.id, "\n".join(lines_q), reply_markup=kb_q)
            return

        # ── Sections list ─────────────────────────────────────
        if action == "sections" and len(parts) >= 4:
            draft_id = parts[2]
            page = int(parts[3]) if parts[3].isdigit() else 0
            draft = base.get_draft(draft_id)
            if not draft:
                ...
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
                return
            sec_rows = list_sections(draft_id)
            lines_s = [f"<b>Sections</b>", f"Draft: <code>{draft_id}</code>", ""]
            if sec_rows:
                for row in sec_rows:
                    lines_s.append(
                        f"• <b>{base.html_escape(row['title'])}</b> — Q{row['start_q']}–Q{row['end_q']}"
                        + (f" — {row['question_time']} sec" if row["question_time"] else "")
                    )
            else:
                lines_s.append("No sections configured.")
            lines_s.append("")
            lines_s.append("Use <code>/section {code} 1-10 | Section Name | 30</code> to add sections.")
            lines_s.append("Use <code>/clearsections {code}</code> to remove all sections.")
            kb_s = InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back to Draft", callback_data=f"ap:view:{draft_id}:{page}")]])
            await base.panel_show_message(query.message, user.id, "\n".join(lines_s), reply_markup=kb_s)
            return

        # ── HTML Export ───────────────────────────────────────
        if action == "htmlexport" and len(parts) >= 4:
            draft_id = parts[2]
            page = int(parts[3]) if parts[3].isdigit() else 0
            draft = base.get_draft(draft_id)
            if not draft:
            ...
            q_rows = base.get_draft_questions(draft_id)
            if not q_rows:
                text, kb = _build_draft_detail_text_markup(user.id, draft_id, page=page, header="⚠️ This draft has no questions. Add questions first.")
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
                return
            try:
                # FIXED: title is properly extracted from draft object before use
                draft_title = base.normalize_visual_text(str(draft["title"]))
                html_content = _draft_quiz_html(dict(draft), [dict(q) for q in q_rows])
                safe_name = base.pdf_safe_filename(draft_title) if hasattr(base, "pdf_safe_filename") else re.sub(r"[^\w\-]", "_", draft_title)[:40]
                filename = f"{safe_name}_{draft_id}_quiz.html"
                await context.bot.send_document(
                    chat_id=user.id,
                    document=InputFile(base.io.BytesIO(html_content.encode("utf-8")), filename=filename),
                    caption=(
                        f"🌐 <b>{base.html_escape(draft_title)}</b>\n"
                        f"Interactive HTML quiz — open in any browser.\n"
                        f"✅ Math/LaTeX rendered · Bengali supported · {len(q_rows)} questions"
                    ),
                    parse_mode=ParseMode.HTML,
                )
                text, kb = _build_draft_detail_text_markup(user.id, draft_id, page=page, header=f"✅ HTML export sent — {len(q_rows)} questions.")
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
            except Exception as exc:
                base.logger.exception("HTML export failed for draft %s: %s", draft_id, exc)
                text, kb = _build_draft_detail_text_markup(user.id, draft_id, page=page, header=f"⚠️ HTML export failed: {base.html_escape(str(exc)[:120])}")
                await base.panel_show_message(query.message, user.id, text, reply_markup=kb)
            return
    ...
    if chat.type == "private" and base.is_bot_admin(user.id):
        ...
        if draft_id and message.poll.type == Poll.QUIZ and message.poll.correct_option_id is not None:
            ...
            ok, q_no = dedup_add_question_to_draft(
                draft_id, cleaned_question, cleaned_options,
                int(message.poll.correct_option_id), cleaned_expl,
                "quizbot_clone" if clone else "forwarded_quiz",
            )
            header = f"✅ {'Clone' if clone else 'Draft'} updated. Added Q{q_no}" if ok else "ℹ️ Duplicate question skipped."
...
async def import_text_into_draft(message, context, draft_id: str, text: str, src: str = "text") -> None:
    ...
    if not parsed:
        await base.safe_reply(
            message,
            "No valid questions were found. Supported format: one question block with options, "
            "and the correct option marked with ✅ or an Answer: line.",
        )
        return
    added = 0
    skipped = 0
    for item in parsed:
        ok, _q_no = dedup_add_question_to_draft(
            draft_id, item["question"], list(item["options"]),
            int(item["correct_option"]), str(item.get("explanation") or ""), src,
        )
        if ok:
            added += 1
        else:
    ...
    await base.send_draft_card(
        context, message.chat.id, message.from_user.id, draft_id,
        header=f"✅ Import complete. Added: <b>{added}</b> | Skipped duplicates: <b>{skipped}</b>",
    )
    draft = base.get_draft(draft_id)
    if draft:
...
# ── create_session_from_draft patch ───────────────────────────
...
async def begin_or_advance_exam(context, session_id: str) -> None:
    ...
    show_title = 1 if not draft_row else int(draft_row.get("show_title_prefix") or 1)
...
async def send_private_results(context, session_id: str) -> None:
    ...
    for p in participants:
        ...
        answers = base.DBH.fetchall(
            "SELECT * FROM answers WHERE session_id=? AND user_id=? ORDER BY q_no", (session_id, p["user_id"])
        )
        ...
        percentile = (
            ((total_users - int(rank_item["rank"])) / (total_users - 1)) * 100.0
            if total_users > 1 else 100.0
        )

        sections = _section_breakdown_for_user(session_id, int(p["user_id"]))
        sec_lines: List[str] = []
        if len(sections) > 1:
            sec_lines.append("\n<b>Section Breakdown</b>")
            for sec in sections:
                sec_lines.append(
                    f"• {base.html_escape(sec['title'])}: ✅{sec['correct']} ❌{sec['wrong']} ➖{sec['skipped']} Score:{sec['score']}"
                )
        ...
        text = (
            ...
            f"Negative / wrong: <b>{neg}</b>\n"
            f"✅ Correct: <b>{correct}</b>  ❌ Wrong: <b>{wrong}</b>  — Skipped: <b>{rank_item['skipped']}</b>\n"
            f"🎯 Accuracy: <b>{accuracy:.2f}%</b>  🏁 Percentage: <b>{percentage:.2f}%</b>\n"
            f"🏆 Percentile: <b>{percentile:.2f}</b>  ⏱ Time: <b>{rank_item.get('time_label','—')}</b>"
            + ("".join(sec_lines) if sec_lines else "")
            + f"\n\n<b>Correct</b>\n{', '.join(correct_links) or '—'}"
            f"\n\n<b>Wrong</b>\n{', '.join(wrong_links) or '—'}"
            f"\n\n<b>Skipped</b>\n{', '.join(skipped_links) or '—'}"
        )
        try_again_kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Try Again", callback_data=f"practice_retry:{session_id}")]])
...
async def _stop_current_poll_and_jobs(context, session: Any) -> None:
    ...
    for job in list(context.job_queue.get_jobs_by_name(f"close:{sid}:{current_index}")):
        job.schedule_removal()
    for job in list(context.job_queue.jobs()):
        if job.name and (str(job.name).startswith(f"advance:{sid}") or str(job.name).startswith(f"close:{sid}:")):
            job.schedule_removal()
...
async def send_admin_pdf_report(context, session_id: str, ranking: List[Dict[str, Any]]) -> None:
    ...
    if hasattr(base, "get_user_theme"):
        ...
        theme = getattr(base, "BUILTIN_THEMES", {
            "midnight": {"bg": "#03101F", "text": "#EAF2FF", "muted": "#B9C7DD", "table": "#07162D",
                         "card1": "#132744", "card2": "#0E2037", "subtext": "#C8D8F4",
                         "accent": "#D7F7CC", "footer": "#95A0B4", "outline": "#18324B"}
        })["midnight"]
    ...
    session_title = base.normalize_visual_text(session["title"])
    pdf_bytes = await asyncio.to_thread(base.render_report_pdf, session_title, summary, compact, theme)
    # use the improved _report_html from this overlay
    html_doc = _report_html(session_title, summary, compact, theme)
    ...
    safe_name = base.pdf_safe_filename(session_title) if hasattr(base, "pdf_safe_filename") else re.sub(r"[^\w\-]", "_", session_title)[:40]
    ...
    for uid in recipients:
        try:
            ...
            await context.bot.send_document(
                uid,
                document=InputFile(base.io.BytesIO(pdf_bytes), filename=f"{safe_name}_report.pdf"),
                caption=f"📄 {base.html_escape(session_title)} — analysis report",
                parse_mode=ParseMode.HTML,
                **kwargs,
            )
            await context.bot.send_document(
                uid,
                document=InputFile(base.io.BytesIO(html_doc.encode("utf-8")), filename=f"{safe_name}_report.html"),
                caption="🌐 Professional HTML report — open in any browser. Supports Math/LaTeX rendering.",
            )
        except TelegramError as exc:
            base.logger.warning("Could not send report to %s: %s", uid, exc)
...
async def handle_text(update: Update, context) -> None:
    ...
    # ── inline state editing from panel ─────────────────────
    if chat.type == "private" and state in {"adv_edit_title", "adv_edit_time", "adv_edit_neg"} and not cmd:
        ...
        if state == "adv_edit_time":
            if not raw.isdigit():
                await base.safe_reply(message, "Send the time in seconds only (e.g. 30).")
                return
            secs = max(5, int(raw))
        ...
        if state == "adv_edit_neg":
            try:
                ...
                await base.safe_reply(message, "Send a valid decimal value (e.g. 0.25).")
                return
    ...
    if chat.type == "private" and state == "adv_await_clone_source" and not cmd:
        ...
        if not token:
            await base.safe_reply(
                message,
                "Send a valid @QuizBot inline text like <code>@QuizBot quiz:ABCDE</code> or any message containing <code>quiz:ABCDE</code>.",
                parse_mode=ParseMode.HTML,
            )
            return
        ...
        base.clear_user_state(user.id)
        await base.send_draft_card(
            context, user.id, user.id, draft_id,
            header=(
                "✅ Clone draft created.\n"
    ...
    if chat.type == "private" and base.user_has_staff_access(user.id):
            ...
            if raw:
                ...
                if token:
                    ...
                    await base.send_draft_card(
                        ...
                        header=(
                            "✅ Clone draft created.\n"
                            "Forward the quiz polls from @QuizBot to this bot inbox.\n"
                            "Use /cloneend when finished."
                        ),
                    )
                    return
        ...
        if cmd == "creator":
            ...
            role = "owner" if base.is_owner(int(draft["owner_id"])) else (
                "all-access admin" if getattr(base, "is_all_access_admin", lambda _x: False)(int(draft["owner_id"]))
                else "admin"
            )
            text = (
                f"<b>Creator Info</b>\n"
        ...
        if cmd == "settitle":
            ...
            if not title_part:
                await base.safe_reply(message, "Title cannot be empty.")
                return
            base.DBH.execute("UPDATE drafts SET title=?, updated_at=? WHERE id=?", (title_part, base.now_ts(), draft["id"]))
            await base.safe_reply(message, f"✅ Title updated to: <b>{base.html_escape(title_part)}</b>", parse_mode=ParseMode.HTML)
            return
        ...
        if cmd == "settime":
            ...
            await base.safe_reply(message, f"✅ Default time set to <b>{secs} sec</b>.", parse_mode=ParseMode.HTML)
            return
        ...
        if cmd == "setneg":
            ...
            try:
                ...
                await base.safe_reply(message, "Invalid value. Example: 0.25")
                return
            base.DBH.execute("UPDATE drafts SET negative_mark=?, updated_at=? WHERE id=?", (neg, base.now_ts(), draft["id"]))
            await base.safe_reply(message, f"✅ Negative mark set to <b>{neg}</b>.", parse_mode=ParseMode.HTML)
            return
        ...
        if cmd == "shuffle":
            ...
            await base.safe_reply(message, f"✅ Questions shuffled for <code>{draft['id']}</code>.", parse_mode=ParseMode.HTML)
            return

        if cmd == "delq":
            parts_raw = args.strip().split(None, 1)
            if len(parts_raw) < 2:
                await base.safe_reply(message, "Usage: /delq DRAFTCODE 3,5-7")
                return
            draft = resolve_editable_draft(user.id, parts_raw[0])
            if not draft:
                await base.safe_reply(message, "Draft not found, or you do not have access.")
                return
            q_nos = parse_q_number_list(parts_raw[1])
            if not q_nos:
                await base.safe_reply(message, "No valid question numbers found.")
                return
            removed = delete_question_numbers(draft["id"], q_nos)
            await base.safe_reply(message, f"✅ Removed <b>{removed}</b> question(s) from <code>{draft['id']}</code>.", parse_mode=ParseMode.HTML)
            return

        if cmd == "section":
            bits = [x.strip() for x in args.strip().split("|")]
            time_part = bits[2] if len(bits) >= 3 else ""
            title_p = bits[1] if len(bits) >= 2 else ""
            if len(bits) < 2:
            ...
            set_section(draft["id"], int(a), int(b), title_p or f"Section {a}-{b}", q_time)
    ...
    # ── private pause / resume / skip ────────────────────────
    if chat.type == "private" and cmd in {"pauseq", "resumeq", "skipq"}:
        session = base.get_active_session(user.id)
        if cmd == "resumeq":
            paused = base.DBH.fetchone(
                "SELECT * FROM sessions WHERE chat_id=? AND status='paused' ORDER BY started_at DESC LIMIT 1", (user.id,)
            )
            if not paused:
        ...
        if cmd == "skipq":
            ...
            await base.safe_reply(message, "⏭ Question skipped.")
            return

    # ── group pause / resume / skip / speed ──────────────────
    if chat.type in {"group", "supergroup"} and cmd in {"pauseq", "resumeq", "skipq", "speed"}:
        ...
        if cmd == "resumeq":
            paused = base.DBH.fetchone(
                "SELECT * FROM sessions WHERE chat_id=? AND status='paused' ORDER BY started_at DESC LIMIT 1", (chat.id,)
            )
            if not paused:
            ...
            await base.safe_reply(message, "▶️ Exam resumed. Next question coming now.")
            return
        ...
        if cmd == "speed":
            ...
            await base.safe_reply(message, f"⚙️ Speed set to <b>{mode_name}</b>. Applies from the next question.", parse_mode=ParseMode.HTML)
            return
...
def owner_private_commands() -> List[BotCommand]:
    return admin_private_commands() + [
        ...
        BotCommand("restart", "Restart bot process"),
    ]
