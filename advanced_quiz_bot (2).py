...
def _fmt_duration(seconds: int) -> str:
    """Format seconds as human-readable duration. Shows min+sec if >= 60s."""
...
def _calculate_student_time(
    answer_by_q: Dict[int, Any],
    q_map: Dict[int, Any],
    question_time_default: int,
) -> Tuple[int, int, int, int]:
    """
    Calculate accurate student exam time using question open/close timestamps.

    Returns: (student_active_time, total_exam_time, avg_per_q, answered_count)

    Logic:
      - actual_exam_window  = MAX(close_ts) - MIN(open_ts) across all questions
        (this equals the sum of all question time-limits, so it is always <= N*T)
      - Skipped questions are *deducted* from the window because the student did
        not spend time on them (per spec: "skip গুলো কাউন্ট হবে না").
      - If exact answer timestamps are present in the answers table we use them
        for a per-question breakdown; otherwise we fall back to the window method.
    """
    answered_q_nos = set(answer_by_q.keys())
    q_time_default = max(1, int(question_time_default))

    # ── Build per-question time limits ──────────────────────────────────────
    q_limits: Dict[int, int] = {}
    for q_no, q in q_map.items():
        q_limits[q_no] = max(1, int(q.get("question_time_override") or q_time_default))

    total_exam_time = sum(q_limits.values())

    # ── Try to get actual_exam_window from DB timestamps ────────────────────
    open_ts_list  = [int(q["open_ts"])  for q in q_map.values() if q.get("open_ts")]
    close_ts_list = [int(q["close_ts"]) for q in q_map.values() if q.get("close_ts")]
    if open_ts_list and close_ts_list:
        actual_exam_window = max(close_ts_list) - min(open_ts_list)
        # Sanity-cap: window must not exceed total question time
        actual_exam_window = min(actual_exam_window, total_exam_time)
    else:
        actual_exam_window = total_exam_time

    # ── Attempt per-question time via answer timestamps ──────────────────────
    per_q_known: Dict[int, int] = {}
    for q_no, ans in answer_by_q.items():
        q = q_map.get(q_no)
        if not q or not q.get("open_ts"):
            continue
        ans_ts = None
        for field in ("answered_at", "ts", "created_at", "answer_ts"):
            val = ans.get(field)
            if val:
                ans_ts = int(val)
                break
        if ans_ts:
            taken = max(1, min(ans_ts - int(q["open_ts"]), q_limits.get(q_no, q_time_default)))
            per_q_known[q_no] = taken

    if len(per_q_known) == len(answered_q_nos) and answered_q_nos:
        # We have exact times for every answered question → use them
        student_active_time = sum(per_q_known.values())
    else:
        # Fallback: exam_window minus time for unanswered (skipped) questions
        skip_time = sum(q_limits[q_no] for q_no in q_map if q_no not in answered_q_nos)
        student_active_time = max(0, actual_exam_window - skip_time)

    answered_count = len(answered_q_nos)
    avg_per_q = (student_active_time // answered_count) if answered_count else 0
    return student_active_time, total_exam_time, avg_per_q, answered_count
...
async def send_private_results(context, session_id: str) -> None:
    ...
    # Fetch question rows WITH timing columns
    qrows = base.DBH.fetchall(
        "SELECT q_no, message_id, open_ts, close_ts, question_time_override "
        "FROM session_questions WHERE session_id=? ORDER BY q_no",
        (session_id,),
    )
    q_map = {int(r["q_no"]): dict(r) for r in qrows}
    ...
    question_time_default = int(session["question_time"] or 30)
    neg_mark = session.get("negative_mark", 0)
    ...
    for p in participants:
        ...
        answers = base.DBH.fetchall(
            "SELECT * FROM answers WHERE session_id=? AND user_id=? ORDER BY q_no",
            (session_id, p["user_id"]),
        )
        answer_by_q = {int(a["q_no"]): dict(a) for a in answers}

        # ── Build Q-links ────────────────────────────────────────────────────
        correct_links: List[str] = []
        wrong_links:   List[str] = []
        skipped_links: List[str] = []
        for q_no, q in sorted(q_map.items()):
        ...
        accuracy  = (correct / max(1, attempted)) * 100.0
        percentage = (correct / max(1, total_questions)) * 100.0
        percentile = (
            100.0 if total_users <= 1
            else ((total_users - int(rank_item["rank"])) / (total_users - 1)) * 100.0
        )

        # ── Accurate time calculation ────────────────────────────────────────
        student_time, total_exam_time, avg_per_q, _ = _calculate_student_time(
            answer_by_q, q_map, question_time_default
        )

        student_time_str = _fmt_duration(student_time)
        total_time_str   = _fmt_duration(total_exam_time)
        avg_q_str        = _fmt_duration(avg_per_q)

        # Time efficiency: how much of the exam time was used
        time_used_pct = (student_time / max(1, total_exam_time)) * 100.0

        # ── Compose professional message ─────────────────────────────────────
        lines: List[str] = [
            f"📋 <b>{base.html_escape(str(session['title']))}</b>",
            "─────────────────────────",
            "",
            f"🏆 Rank: <b>#{rank_item['rank']}</b> / {total_users}"
            f"   |   ⭐ Score: <b>{rank_item['score']}</b>",
            f"📉 Negative/Wrong: <b>{neg_mark}</b>",
            "",
            "📊 <b>Performance Summary</b>",
            f"✅ Correct : <b>{correct}</b>   ❌ Wrong : <b>{wrong}</b>   ➖ Skipped : <b>{skipped}</b>",
            f"🎯 Accuracy   : <b>{accuracy:.2f}%</b>",
            f"📈 Score %    : <b>{percentage:.2f}%</b>",
            f"🥇 Percentile : <b>{percentile:.2f}</b>",
            "",
            "⏱ <b>Time Analysis</b>",
            f"📌 Total Exam Time  : <b>{total_time_str}</b>  ({total_questions}Q × {question_time_default}s)",
            f"⚡ Your Active Time : <b>{student_time_str}</b>  ({time_used_pct:.0f}% of exam)",
            f"🕐 Avg / Question   : <b>{avg_q_str}</b>",
            "",
        ]

        if correct_links:
            lines.append(f"✅ <b>Correct  ({correct})</b>")
            lines.append(", ".join(correct_links))
            lines.append("")
        if wrong_links:
            lines.append(f"❌ <b>Wrong  ({wrong})</b>")
            lines.append(", ".join(wrong_links))
            lines.append("")
        if skipped_links:
            lines.append(f"➖ <b>Skipped  ({skipped})</b>")
            lines.append(", ".join(skipped_links))
...
async def send_admin_pdf_report(context, session_id: str, ranking: List[Dict[str, Any]]) -> None:
    ...
    for r in ranking:
        ...
        raw_time = r.get("time_label") or r.get("time") or "0s"
        # Re-format using accurate formatter if it is a plain seconds value
        try:
            secs = int(str(raw_time).rstrip("s"))
            raw_time = _fmt_duration(secs)
        except (ValueError, AttributeError):
            pass
        compact.append({**r, "name": name, "sub_name": "", "time": raw_time})
